...
void HttpListener::start(std::uint64_t minimum_runtime_revision) {
    minimum_runtime_revision_.store(minimum_runtime_revision, std::memory_order_release);
    accept_next();
}

void HttpListener::retire(DrainedHandler on_drained) noexcept {
    try {
...
tcp::endpoint HttpListener::local_endpoint() const {
    return endpoint_;
}

void HttpListener::accept_next() {
    ...
    acceptor_.async_accept(
        ...
        guarded_handler(&HttpListener::on_accept));
}

void HttpListener::on_accept(
    ...
    try {
        ...
        const auto session = std::make_shared<http::HttpSession>(
            std::move(socket),
            stopping_,
            runtime_store_,
            listener_port_,
            ...
            [weak_listener = weak_from_this(), permit = std::move(permit)](
                http::HttpSession* closed_session) {
        ...
        if (logger_) {
            try {
                logger_->log_error(
                    ...
                    "http_listener",
                    "could not create an HTTP session: " + std::string{exception.what()});
            } catch (...) {
            }
        }
        schedule_accept_retry();
...
void HttpListener::schedule_accept_retry() {
    if (retired_.load() || stopping_->load() || !acceptor_.is_open()) {
        notify_if_drained();
        return;
    }
    ...
    accept_retry_timer_.async_wait(
        guarded_handler(&HttpListener::on_accept_retry));
}

void HttpListener::on_accept_retry(const boost::system::error_code& error) {
    if (!error) accept_next();
}

void HttpListener::notify_if_drained() {
    if (!retired_.load() || !sessions_.empty() || !on_drained_) return;
    auto on_drained = std::move(on_drained_);
    on_drained(this);
}

void HttpListener::retire_on_executor(DrainedHandler on_drained) {
    retired_.store(true);
...
void HttpListener::stop_on_executor() {
    try {
        accept_retry_timer_.cancel();
    } catch (...) {
    }
...
void HttpListener::emergency_close_noexcept(bool stopping) noexcept {
...
void HttpListener::recover_accept_noexcept() noexcept {
    try {
        schedule_accept_retry();
    } catch (...) {
        emergency_close_noexcept(false);
